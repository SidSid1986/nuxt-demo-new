 ...
 * @LastEditors: Sid Li
 * @LastEditTime: 2026-04-25 13:48:25
...
export function chatSendToWeCom(data: {
  ...
  content: string;
  touser: string;
  chat_type: string; // product / maintain / after_sale
}) {
...
// 产品咨询 - 释放座席
export function releaseProductAgent(data: {
  visitor_id: string;
  agent: number;
}) {
  return request('/api/chat/release-product-agent', {
    method: 'POST',
    body: data,
  });
}
...
// 维保咨询 - 释放座席
export function releaseMaintainAgent(data: {
  visitor_id: string;
  agent: number;
}) {
  return request('/api/chat/release-maintain-agent', {
    method: 'POST',
    body: data,
  });
}
...
// ======================== 售后咨询 - 新增（解决你报错） ========================
// 售后咨询 - 分配座席
export function assignAfterSaleAgent(visitor_id?: string) {
  let url = '/api/chat/assign-after-sale-agent';
  if (visitor_id) url += `?visitor_id=${visitor_id}`;
  return request(url, { method: 'get' });
}

// 售后咨询 - 释放座席
export function releaseAfterSaleAgent(data: {
  visitor_id: string;
  agent: number;
}) {
  return request('/api/chat/release-after-sale-agent', {
    method: 'POST',
    body: data,
  });
}
...
export function chatPollHeartbeat(data: { visitor_id: string; chat_type: string }) {
  return request(
    `/api/chat/poll-heartbeat?visitor_id=${data.visitor_id}&chat_type=${data.chat_type}`,
    {
      method: 'get',
    }
  );
...
export function chatSendToWeComCard(data: {
  visitor_id: string;
  sender: string;
  content: string;
  touser: string;
  ...
  return request('/api/chat/send_to_wecom_card', {
    method: 'POST',
    body: data,
  });
}