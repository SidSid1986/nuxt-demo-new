<template>
  <div class="chat-service">
    ...
    <!-- ============================================== -->
    <!-- 企业微信客服 3 售后咨询（新增，不删一行老代码） -->
    <!-- ============================================== -->
    <div class="chat-group wecom-group">
      <div class="mini-chat wecom-mini" :class="{ blink: wecomBlink3 }" v-show="minimizeWecom3" @click="restoreWecom3">
        <img src="/images/chat/chatLogo.png" alt="企微头像" class="mini-avatar" />
        <span class="mini-title">售后咨询测试h5</span>
        <button class="mini-close" @click.stop="closeMiniWecom3">×</button>
      </div>

      <button class="chat-toggle wecom" @click="handleOpenWecom3">
        {{ openWecom3 ? "关闭售后咨询" : "售后咨询" }}
      </button>

      <div class="chat-panel wecom-panel" v-show="openWecom3 && !minimizeWecom3">
        <div class="chat-header wecom-header">
          <span class="header-title">售后咨询</span>
          <span class="header-status">客服在线</span>
          <div class="window-controls">
            <button class="control-btn minimize-btn" @click="minimizeWecom3 = true">
              <svg width="16" height="2" viewBox="0 0 16 2" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect width="16" height="2" rx="1" fill="white" />
              </svg>
            </button>
            <button class="control-btn close-btn" @click="handleCloseWecom3">
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M12 4L4 12M4 4L12 12" stroke="white" stroke-width="2" stroke-linecap="round" />
              </svg>
            </button>
        ...
        <div class="chat-body" ref="wecomBodyRef3">
          <div class="msg-item admin" v-if="openWecom3">
            <img src="/images/chat/chatLogo.png" alt="企微客服头像" class="msg-avatar" />
            <div class="msg-content">
              <div class="msg-text">
                您好！售后咨询专属客服为您服务，请问有什么可以帮助您的？
              </div>
              <div class="msg-time">{{ getCurrentTime() }}</div>
            </div>
          </div>
          <div v-for="item in wecomMsgList3" :key="item.id" class="msg-item" :class="item.sender">
            <img :src="item.sender === 'visitor'
              ? '/images/chat/wecom.png'
              : '/images/chat/chatLogo.png'
              " alt="头像" class="msg-avatar" />
        ...
        <div class="chat-footer">
          <input v-model="wecomInputText3" @keyup.enter="sendWecomMsg3" placeholder="售后咨询..." class="msg-input" />
          <button @click="sendWecomMsg3" class="send-btn wecom-btn">
            发送
          </button>
        </div>
      </div>
...
import {
  ...
  assignAfterSaleAgent,
  releaseAfterSaleAgent,
  chatPollHeartbeat,
  chatSendToWeComCard
} from "@/server/chat";
...
const hasWarnedAfterSale = ref(false);
let productInactiveTimer = null;
let maintainInactiveTimer = null;
let afterSaleInactiveTimer = null;
...
// ==================== 售后咨询（新增） ====================
const WECOM_TOUSER_3 = "XiaoNanGua"; // 这里改成你售后客服的企业微信ID
const openWecom3 = ref(false);
const minimizeWecom3 = ref(false);
const wecomBlink3 = ref(false);
const wecomInputText3 = ref("");
const wecomMsgList3 = ref([]);
const wecomBodyRef3 = ref(null);

const wecomVisitorId3 = ref("");
let currentAssignedAgent3 = null;

const wecomLastMsgId3 = ref(0);
const isWecomPolling3 = ref(false);
const assignedWecomAgent3 = ref(null);
...
const clearAfterSaleInactiveTimer = () => {
  if (afterSaleInactiveTimer) {
    clearTimeout(afterSaleInactiveTimer);
    afterSaleInactiveTimer = null;
  }
};
...
const resetAfterSaleInactiveTimer = () => {
  clearAfterSaleInactiveTimer();
  hasWarnedAfterSale.value = false;

  if (!openWecom3.value || !wecomVisitorId3.value) return;

  afterSaleInactiveTimer = setTimeout(() => {
    if (openWecom3.value && wecomVisitorId3.value && !hasWarnedAfterSale.value) {
      hasWarnedAfterSale.value = true;
      alert("当前售后咨询会话已长时间未操作，如继续无操作，系统将自动结束会话。");
    }
  }, INACTIVE_WARNING_MS);
};
...
const clearAfterSaleSessionState = () => {
  openWecom3.value = false;
  minimizeWecom3.value = false;
  assignedWecomAgent3.value = null;
  wecomVisitorId3.value = "";
  currentAssignedAgent3 = null;
  clearAfterSaleInactiveTimer();
  hasWarnedAfterSale.value = false;
};
...
// ==================== 售后咨询：打开（新增） ====================
const handleOpenWecom3 = async () => {
  try {
    if (!openWecom3.value) {
      const finalVisitorId = `wecom_after_sale_${vid}`;

      const res = await assignAfterSaleAgent(finalVisitorId);
      if (res.code === 503) {
        alert(res.msg);
        return;
      }

      currentAssignedAgent3 = res.agent;
      wecomVisitorId3.value = finalVisitorId;
      assignedWecomAgent3.value = res.agent;

      openWecom3.value = true;
      open.value = false;
      openWecom.value = false;
      openWecom2.value = false;
      minimizeWecom3.value = false;

      resetAfterSaleInactiveTimer();

      nextTick(() => {
        if (wecomBodyRef3.value) {
          wecomBodyRef3.value.scrollTop = wecomBodyRef3.value.scrollHeight;
        }
      });
    } else {
      openWecom3.value = false;
      if (assignedWecomAgent3.value && wecomVisitorId3.value) {
        try {
          await releaseAfterSaleAgent({
            visitor_id: wecomVisitorId3.value,
            agent: assignedWecomAgent3.value,
          });
        } catch { }
      }
      clearAfterSaleSessionState();
    }
  } catch (e) {
    console.error("分配售后客服失败", e);
  }
};

// ==================== 售后发送（新增） ====================
// ==================== 售后发送（只改这里！使用新卡片接口） ====================
const sendWecomMsg3 = async () => {
  if (!wecomInputText3.value.trim()) return;
  if (!wecomVisitorId3.value) {
    alert("请先分配座席");
    return;
  }

  try {
    // ✅ 只改这一行：使用 chatSendToWeComCard（卡片消息，点击进H5）
    const res = await chatSendToWeComCard({
      visitor_id: wecomVisitorId3.value,
      sender: "visitor",
      content: wecomInputText3.value,
      touser: WECOM_TOUSER_3,
      chat_type: "after_sale",
    });

    if (res?.code === 200) {
      wecomInputText3.value = "";
      resetAfterSaleInactiveTimer();
      return;
    }

    if (res?.code === 403 || res?.code === 503) {
      alert(res.msg || "当前售后咨询会话已长时间未操作，已自动结束，请重新打开咨询窗口");
      clearAfterSaleSessionState();
      return;
    }
    ...
    console.error("售后咨询发送失败", e);
    alert("发送失败");
  }
};

const startWecomLongPoll3 = async () => {
  if (isWecomPolling3.value) return;
  if (!openWecom3.value && !minimizeWecom3.value) {
    setTimeout(startWecomLongPoll3, 2000);
    return;
  }
  isWecomPolling3.value = true;
  try {
    if (wecomVisitorId3.value) {
      const hbRes = await chatPollHeartbeat({
        visitor_id: wecomVisitorId3.value,
      ...
      if (hbRes?.code === 403) {
        alert(hbRes.msg || "当前售后咨询会话长时间未操作，已自动结束，请重新打开咨询窗口");
        clearAfterSaleSessionState();
        return;
      }
      ...
      const res = await chatPullMessageLongPoll(
        wecomVisitorId3.value,
        wecomLastMsgId3.value
      );
      if (res?.code === 200 && res.data?.length > 0) {
        wecomMsgList3.value.push(...res.data);
        wecomLastMsgId3.value = res.data.at(-1).id;
    ...
    isWecomPolling3.value = false;
    setTimeout(startWecomLongPoll3, 100);
  }
};

const handleCloseWecom3 = async () => {
  openWecom3.value = false;
  minimizeWecom3.value = false;
  if (assignedWecomAgent3.value && wecomVisitorId3.value) {
    try {
...
const restoreWecom3 = () => {
  minimizeWecom3.value = false;
  openWecom3.value = true;
  wecomBlink3.value = false;
};

const closeMiniWecom3 = async () => {
  minimizeWecom3.value = false;
  openWecom3.value = false;
  if (assignedWecomAgent3.value && wecomVisitorId3.value) {
    try {
...
watch([openWecom3, minimizeWecom3], ([v]) => {
  if (v) {
    resetAfterSaleInactiveTimer();
  } else {
    clearAfterSaleInactiveTimer();
  }
  if (v && !isWecomPolling3.value) startWecomLongPoll3();
});
